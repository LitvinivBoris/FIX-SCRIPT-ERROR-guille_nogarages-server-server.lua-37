...
ESX.RegisterServerCallback('guille_getvehicles', function(source,cb) 
    ...
    MySQL.Async.fetchAll("SELECT vehicle, position, heading FROM owned_vehicles WHERE owner=@identifier AND stored=0", {
        ['@identifier'] = xPlayer.getIdentifier()
    }, function(result)
        print('ID send to server: ' ..xPlayer.getIdentifier())
        local vehicles = {}
